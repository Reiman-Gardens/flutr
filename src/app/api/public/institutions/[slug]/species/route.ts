...
export async function GET(request: NextRequest, context: RouteContext) {
  try {
    ...
    const baseCondition = eq(butterfly_species_institution.institution_id, institutionId);
    ...
    const [rows, totalResult] = await Promise.all([
      ...
      db.select({ total: count() }).from(butterfly_species_institution).where(baseCondition),
    ]);
